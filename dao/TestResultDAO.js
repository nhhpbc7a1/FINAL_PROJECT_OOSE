...
class TestResultDAO {
    ...
    /**
     * Find test results by record ID
     * @param {number} recordId - Medical record ID
     * @returns {Promise<Array>} List of test results for the record
     */
    static async findByRecordId(recordId) {
        try {
            ...
            console.error(`Error retrieving test results for record ID ${recordId}:`, error);
            throw error;
        }
    }
     ...
     */
    static async getResultsForMedicalRecord(patientId, limit = 20) {
        try {
            return await db('TestResult as tr')
                .join('TestRequest as req', 'tr.requestId', '=', 'req.requestId')
                .leftJoin('Appointment as a', 'req.appointmentId', '=', 'a.appointmentId')
                ...
                .select(
                    'tr.resultId',
                    'tr.performedDate',
                    'tr.resultNumeric as resultValue',
                    'tr.unit as resultUnit',
                    'tr.normalRange',
                    'tr.interpretation',
                    'tr.resultFileId as attachmentUrl',
                    's.name as testName',
                    'du.fullName as doctorName'
                )
                .where('a.patientId', patientId)
